...
with tab1:
    ...
    if len(df) > 0:
        ...
        total_value = pd.to_numeric(
            df.get("contract_value", pd.Series(dtype=float)),
            errors="coerce"
        ).fillna(0).sum()
        ...
        active_count = len(df[df["contract_status"] == "نشط"]) if "contract_status" in df.columns else 0
        unpaid_count = len(df[df["payment_status"] == "غير مسدد"]) if "payment_status" in df.columns else 0
 
        col3.metric("العقود النشطة", active_count)
        col4.metric("غير المسددة", unpaid_count)
...
with tab3:
    ...
    if len(df) == 0:
        ...
        with col_search1:
            filter_contract_status = st.selectbox(
                "حالة العقد",
        ...
        with col_search3:
            filter_city = st.text_input("المدينة", key="filter_city")
        ...
        if len(filtered_df) > 0:
            ...
            elevator_type_options = ["ركاب", "خدمة", "بانوراما", "أخرى"]
            ...
            new_collector = st.selectbox(
                "مسؤول التحصيل",
                ["طه", "أحمد", "آخر"],
                index=["طه", "أحمد", "آخر"].index(safe_text(row.get("collector"))) if safe_text(row.get("collector")) in ["طه", "أحمد", "آخر"] else 0,
                key="edit_collector"
            )
